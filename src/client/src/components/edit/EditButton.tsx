...
export const EditForm = ({ editSubmitHandler, data, deleteHandler }): JSX.Element => {
  ...
  return (
    <Form form={form} name="edit-form" onFinish={editSubmitHandler}>
      ...
      <Form.Item name="language" label="Язык программирования" rules={[{ required: true }]}>
        <Select placeholder="Select your functions language" value={state.language} defaultValue={state.language}>
          {languages.map(x => (
      ...
      <Form.Item name="code" label="Исходный код функции" rules={[{ required: true }]}>
        <TextArea style={{ height: 150 }} value={state.code} defaultValue={state.code} />
      </Form.Item>
      <Form.Item name="tags" label="Теги">
        <EditableTagGroup tags={state.tags} onChange={setTags} />
      </Form.Item>
      <Form.Item>
        <Button type="primary" htmlType="submit" block style={{ margin: '10px' }}>
  ...
  return (
    <div className="edit-buttons" style={{ display: 'inline-block' }}>
      ...
      <Modal
        ...
        footer={[]}
      >
        <EditForm data={data} deleteHandler={deleteHandler} editSubmitHandler={editSubmitHandler} />
      </Modal>
    </div>
  );
};
